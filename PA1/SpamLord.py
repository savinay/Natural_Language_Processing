...
def process_file(name, f):
    ...
    for line in f:
        ...
        phone_matches = re.findall(phone_exp,line)
        # print matches
        for m in email_matches:
            ...
            if m[0] != 'Server':
                ...
                if m[0] != '':
                    email = '%s@%sedu' % (m[0].replace('-',''), m[2].replace(';','.'))
                    if '-' in m[2]:
                        a = m[2].replace('-','')
                        email = '%s@%sedu' % (m[0].replace('-',''), a)
                    if '.' not in m[2] and ';' not in m[2]:
